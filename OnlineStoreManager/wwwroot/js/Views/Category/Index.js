...
const CATEGORY_PAGE = {
    setModificationContent(_action) {
        ...
        var categoryId = w2ui['_grid'].getSelection()[0];
        if (_action == 'Update' && !categoryId)
            return;

        var data = {
            action: _action,       // hardcode enum Repository.CRUD
            category: {
                id: _action == 'Update' ? categoryId : null,// get category ID if action is update
            }
        };
        ...
        var that = this;
        $.post('/Category/Modify', data, function (requestModifyForm) {
