...
const IMPORT_ORDER_PAGE = {
    setModificationContent(_action) {
        // request form content
        // vi validation phia server
        var importOrderId = w2ui['_grid'].getSelection()[0];
        if (_action == 'Update' && !importOrderId)
            return;

        var data = {
            action: _action,       // hardcode enum Repository.CRUD
            importOrder: {
                id: _action == 'Update' ? importOrderId : null,// get importOrder ID if action is update
            }
        };
        ...
        var that = this;
        $.post('/ImportOrder/Modify', data, function (requestModifyForm) {
    ...
    edit: function () {
        ...
        var importOrder = {
            id: recs.id,
            discount: recs.discount,
            expecteddeliverydate: recs.expecteddeliverydate,
            warehouseid: recs.warehouseid,
            importstatusid: recs.importstatusid,
            canceldescription: recs.canceldescription,
            description: recs.description,
        };
        $.post('/ImportOrder/Update', {
